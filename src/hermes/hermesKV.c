...
hermes_exec_ack(spacetime_ack_t *ack_ptr, struct mica_op *kv_ptr,
				spacetime_group_membership curr_membership,
				spacetime_op_t *read_write_op, async_ctx *a_ctx)
{
						  ...
						  lock_free_read_meta.last_local_write_ts.tie_breaker_id))
	{
							  ...
							  curr_meta->last_local_write_ts.tie_breaker_id ))
		{
			...
			async_set_ack_ts(a_ctx, NULL, curr_meta->op_buffer_index, ack_ptr->sender, 0);
            async_complete_read(a_ctx, &read_write_op[0], 0);
            stop_latency_of_completed_reads(read_write_op,
                                            a_ctx->worker_lid,
                                            a_ctx->stopwatch_for_req_latency);
#endif
...
hermes_exec_dispatcher(enum hermes_batch_type_t type, void* op_ptr, struct mica_op *kv_ptr,
					   spacetime_group_membership curr_membership,
					   uint8_t idx, spacetime_op_t *read_write_op, async_ctx *a_ctx)
{
	switch (type){
		case local_ops:
		...
		case acks:
		    if(ENABLE_RMWs == 0)
                hermes_exec_ack(op_ptr, kv_ptr, curr_membership, read_write_op, a_ctx);
		    else {
		...
		if(kv_ptr[I] != NULL) {
			...
			if(key_ptr_log[1] == key_ptr_req[0]){ //Key Found 8 Byte keys
				...
				hermes_exec_dispatcher(type, op_ptr, kv_ptr[I], curr_membership,
									   (uint8_t) I, read_write_ops, (async_ctx *) a_ctx);
			}
		}

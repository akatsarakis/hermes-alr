...
//			print_total_send_recv_msgs_n_credits(&inv_ud_c, &ack_ud_c, &val_ud_c, &crd_ud_c);
        ...
        if(!ENABLE_HADES_FAILURE_DETECTION || fd_warmup_time_has_passed == 1){
            ...
            node_suspected = refill_ops(&trace_iter, worker_lid, trace, ops,
                                        ...
                                        n_hottest_keys_in_ops_get, n_hottest_keys_in_ops_put, &a_ctx);

            hermes_batch_ops_to_KVS(local_ops, (uint8_t *) ops, max_batch_size, sizeof(spacetime_op_t),
                                    group_membership, NULL, NULL, (uint8_t) worker_lid, NULL);

#ifdef ENABLE_ASYNC_ONLY_READS
            async_set_ops_ts(&a_ctx, ops);
#else
            ...
            if (update_ratio > 0) {
                ...
                if(invs_polled > 0) {
                    hermes_batch_ops_to_KVS(invs, (uint8_t *) inv_recv_ops, invs_polled, sizeof(spacetime_inv_t),
                                            group_membership, &node_suspected, ops, (uint8_t) worker_lid, NULL);
                ...
                if(has_outstanding_vals == 0 && has_remaining_vals_from_memb_change == 0) {
                    ...
                    if (acks_polled > 0){
                        hermes_batch_ops_to_KVS(acks, (uint8_t *) ack_recv_ops, acks_polled, ack_size,
                                                group_membership, NULL, ops, (uint8_t) worker_lid, &a_ctx);

								...
								struct timespec *stopwatch)
{
    ...
    if (machine_id == 0 && worker_lid == worker_measuring_latency)
        if(ops[0].op_meta.opcode == ST_OP_GET &&
           (ops[0].op_meta.state == ST_MISS || ops[0].op_meta.state == _ST_GET_COMPLETE))
            stop_latency_measurment(ops[0].op_meta.opcode, stopwatch);
}
...
refill_ops(uint32_t *trace_iter, uint16_t worker_lid,
           ...
           spacetime_op_t **n_hottest_keys_in_ops_put, async_ctx* a_ctx)
{
	static uint8_t first_iter_has_passed[MAX_WORKERS_PER_MACHINE] = { 0 };

#ifdef ENABLE_ASYNC_ONLY_READS
	...
	for(int i = 0; i < max_batch_size; i++) {
		if(ENABLE_ASSERTIONS && first_iter_has_passed[worker_lid] == 1){
				...
				assert(ops[i].op_meta.state == ST_PUT_COMPLETE ||
					   ...
					   ops[i].op_meta.state == ST_GET_ASYNC_COMPLETE ||
...
# ifdef ENABLE_READ_BUFFER
        if (first_iter_has_passed[worker_lid] != 0) {
		    if (i != 0) // avoid buffering requests that we measure latency
		        try_add_op_to_read_buf(a_ctx, &ops[i], i);
        }
# endif
#endif
		...
		if (first_iter_has_passed[worker_lid] == 0 ||
			...
			ops[i].op_meta.state == _ST_GET_COMPLETE)
		{
            if (first_iter_has_passed[worker_lid] != 0) {
                ...
                if(ops[i].op_meta.state == ST_PUT_COMPLETE)
                    w_stats[worker_lid].completed_wrs_per_worker += ENABLE_COALESCE_OF_HOT_REQS ? ops[i].no_coales : 1;
            ...
            ops[i].op_meta.state = ST_NEW;
#ifdef ENABLE_LAST_BUFF_WRITE
            ops[i].op_meta.opcode = (uint8_t) (i + 1 == max_batch_size
                                               ? ST_OP_PUT
            ...
            uint8_t is_upd = ops[i].op_meta.opcode == ST_OP_PUT || ops[i].op_meta.opcode == ST_OP_RMW;

#ifdef ENABLE_ASYNC_ONLY_READS
			async_reset_op_ts(a_ctx, i);
#endif
            if (is_upd){
